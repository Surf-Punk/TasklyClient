...
export default function Tasks() {
    ...
    const handleOrderBy = (value) => {
        const currentDirection = searchParams.get('orderBy') === value && sortDirection === 'asc' ? 'desc' : 'asc';
        searchParams.set('orderBy', value);
        setSortDirection(currentDirection); 
        searchParams.set('sortDirection', currentDirection);
        setSearchParams(searchParams);
    };

    useEffect(() => {
        const fetchTasks = async () => {
            //const query = searchParams.size ? `?${searchParams.toString()}&sortDirection=${sortDirection}` : '';
            const query = searchParams.append.length ? `?${searchParams.toString()}` : '';
        ...
        <Box p="5" maxW="3lg" mx="auto">
            ...
            <TableContainer overflowX="auto">
                ...
                <Flex onClick={() => handleOrderBy('priority')} cursor='pointer' alignItems='center'>
                    Priority {searchParams.get('orderBy') === 'priority' && (sortDirection === 'asc' ? <BsArrowUp /> : <BsArrowDown />)}
                </Flex>
                </Th>
                <Th>
